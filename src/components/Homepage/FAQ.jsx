...
export default function FAQ() {
  ...
  return (
    <section className="relative py-16 md:py-32 bg-slate-50 overflow-hidden font-['Inter']">
      ...
      <div
        className="absolute w-[300px] md:w-[600px] h-[300px] md:h-[600px] bg-blue-500/5 rounded-full blur-[80px] md:blur-[120px] pointer-events-none transition-transform duration-700 ease-out hidden sm:block"
      ...
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12 text-center mb-16 md:mb-24 relative z-10">
        <div className="flex items-center justify-center gap-2 mb-4 md:mb-6">
           <MapPin className="text-blue-500 w-3 h-3 md:w-4 md:h-4" />
        ...
        <h2 className="text-3xl md:text-5xl font-bold text-slate-900 mb-6 md:mb-8 uppercase tracking-tighter italic leading-tight md:leading-none">
          Technical <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-500 to-red-600">Support & FAQ</span>
        </h2>
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-16">
        <div className="grid grid-cols-1 2xl:grid-cols-2 gap-4 md:gap-6 items-start">
          {faqs.map((faq, idx) => (
            <div
              key={idx}
              className={`group bg-white rounded-xl md:rounded-2xl border transition-all duration-500 hover:border-blue-500/40 ${
                openIndex === idx ? 'border-blue-500/30 shadow-2xl shadow-blue-500/10' : 'border-slate-200 shadow-sm'
              }`}
            >
              <button
              ...
              >
                <div className="flex gap-3 md:gap-5">
                   <HelpCircle className={`w-6 h-6 md:w-7 md:h-7 shrink-0 mt-0.5 ${openIndex === idx ? 'text-blue-500' : 'text-slate-300'}`} />
                ...
                <div className={`flex-shrink-0 w-10 h-10 md:w-12 md:h-12 rounded-lg md:rounded-xl flex items-center justify-center transition-all duration-500 ${
                  openIndex === idx ? 'bg-blue-600 rotate-180 text-white shadow-lg shadow-blue-600/30' : 'bg-slate-50 text-slate-400'
                }`}>
              ...
              >
                <div className="text-slate-500 text-sm md:text-lg font-normal leading-relaxed italic border-l-2 md:border-l-4 border-blue-500/20 pl-4 md:pl-8">
                  {faq.answer}
                </div>
              </div>
            </div>
      ...
      <div className="mt-16 md:mt-32 max-w-[1800px] mx-auto px-6 lg:px-16 relative z-10">
        <div className="bg-slate-900 rounded-[2rem] md:rounded-[4rem] border border-white/5 shadow-2xl overflow-hidden relative p-8 md:p-12 lg:p-24 flex flex-col lg:flex-row items-center justify-between gap-10 md:gap-12">
          <div className="absolute top-0 right-0 w-[300px] md:w-[500px] h-[300px] md:h-[500px] bg-blue-600/10 rounded-full blur-[100px] md:blur-[150px]"></div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6 md:gap-8 relative z-10 w-full lg:w-auto">
            <button
              ...
              className="w-full sm:w-auto px-10 md:px-16 py-4 md:py-6 bg-blue-600 text-white rounded-xl md:rounded-2xl font-bold uppercase text-xs md:text-sm tracking-widest transition-all duration-300 hover:bg-blue-500 hover:scale-105 active:scale-95 shadow-2xl shadow-blue-600/40"
            >
              Request B2B Pricing
            </button>
            <div className="h-[1px] w-12 bg-white/10 hidden sm:block"></div>
            <a href="tel:+919443439096" className="text-white font-bold uppercase text-xs md:text-sm tracking-[0.2em] md:tracking-[0.3em] border-b-2 border-blue-500 pb-2 hover:text-blue-500 transition-colors text-center">
