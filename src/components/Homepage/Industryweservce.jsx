...
const industries = [
  {
    ...
    icon: Mountain,
    gradient: "from-blue-600 to-blue-700" // Unified to single brand color
  },
  {
        ...
        <div className="grid lg:grid-cols-2 gap-8 md:gap-12 items-end mb-16 md:mb-32">
          <div>
            <div className="flex items-center gap-4 mb-6 md:mb-8">
              <div className="h-[2px] md:h-[3px] w-12 md:w-16 bg-blue-600"></div> {/* Single Color Accent */}
              <span className="text-blue-600 font-['Poppins'] font-semibold tracking-[0.3em] md:tracking-[0.4em] uppercase text-[9px] md:text-xs">
          ...
          <div className="lg:pb-12">
            <p className="text-lg lg:text-2xl max-w-2xl text-slate-500 italic border-l-4 md:border-l-8 border-blue-600 pl-6 md:pl-8 leading-relaxed md:leading-tight font-normal"> {/* Unified Border Color */}
          ...
          {industries.map((item, index) => (
            ...
            >
              <div className="relative h-64 md:h-80 overflow-hidden">
                ...
                />
                <div className={`absolute inset-0 bg-blue-600 opacity-0 group-hover:opacity-40 transition-opacity duration-700`}></div>
                ...
                <div className="absolute bottom-6 left-6 p-3 md:p-4 bg-white shadow-xl border border-slate-100 group-hover:bg-slate-900 group-hover:border-slate-800 transition-all duration-500">
                   <item.icon className="w-5 h-5 md:w-6 md:h-6 text-blue-600 group-hover:text-white group-hover:rotate-[360deg] transition-all duration-700" /> {/* Unified Icon Color */}
                </div>
                
                <div className="absolute top-6 right-6 p-2 md:p-3 bg-white text-slate-900 shadow-lg lg:translate-y-4 opacity-100 lg:opacity-0 lg:group-hover:opacity-100 lg:group-hover:translate-y-0 transition-all duration-500 delay-100 hover:bg-blue-600 hover:text-white">
              ...
              <div className="p-8 md:p-10 relative">
                <div className="flex items-center gap-2 mb-4">
                  <div className={`w-2 h-2 rounded-full bg-blue-600`}></div> {/* Unified Dot Color */}
                  <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] text-blue-600"> {/* Unified Category Color */}
                    {item.category}
                  </span>
                </div>
                <h3 className="text-xl md:text-2xl font-['Poppins'] font-bold uppercase tracking-tight text-slate-900 group-hover:text-blue-600 transition-all duration-500 mb-4 md:mb-6 leading-none">
                  {item.title}
                </h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {item.tags.map((tag, idx) => (
                    <span key={idx} className="text-[8px] md:text-[9px] font-semibold uppercase tracking-wider px-2 md:px-3 py-1 bg-slate-50 border border-slate-100 text-slate-400 group-hover:border-blue-200 group-hover:text-blue-600 transition-colors">
                      {tag}
                    </span>
                  ))}
                </div>
              </div>
              <div className={`absolute bottom-0 left-0 h-1 md:h-1.5 w-0 bg-blue-600 transition-all duration-[1s] ease-out group-hover:w-full`}></div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-16 md:mt-32 pt-12 md:pt-16 border-t border-slate-100 flex flex-col xl:flex-row justify-between items-center gap-8 md:gap-12">
          <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 text-center md:text-left">
            <div className="p-3 md:p-4 bg-blue-50 rounded-full">
              <MapPin className="text-blue-600 w-6 h-6 md:w-8 md:h-8 animate-pulse" />
            </div>
            <div>
          ...
          <button 
            onClick={() => navigate('/contactus')}
            className="w-full md:w-auto relative overflow-hidden group/btn px-10 md:px-16 py-5 md:py-6 bg-slate-900 text-white font-['Poppins'] font-bold uppercase text-[10px] md:text-xs tracking-[0.2em] md:tracking-[0.3em] transition-all hover:bg-blue-600 active:scale-95 shadow-2xl"
          >
